/* =================================================================
   ...
   structure (index.html) et des styles (sg-pc.css).
   ================================================================= */
window.createSgPcComponent = function (DCLogic) {
  return class Component extends DCLogic {
    ...
    componentDidMount() {
      ...
      // Filet de sécurité : quoi qu'il arrive, la page est révélée au bout de 6 s.
      // Évite de rester bloqué zoomé/flouté si l'intro ne démarre jamais.
      this._introFallback = setTimeout(() => {
        if (!this.state.revealed) this.finishIntro();
      }, 6000);
    ...
    componentWillUnmount() {
      ...
      clearTimeout(this._introFallback);
    ...
    startIntro(attempt = 0) {
      const canvas = this.canvasRef;
      if (!canvas) {
        // Le canvas n'est pas encore monté : on réessaie quelques frames
        // avant d'abandonner (et de révéler la page malgré tout).
        if (attempt < 5) { requestAnimationFrame(() => this.startIntro(attempt + 1)); return; }
        this.finishIntro();
        return;
      }
      if (this._introStarted) return;   // ne démarre l'intro qu'une seule fois
    ...
    finishIntro() {
      if (this.state.revealed) return;   // déjà révélée : on ne refait rien
      clearTimeout(this._introFallback);
      cancelAnimationFrame(this.raf);
    ...
    submitQuote = async () => {
      ...
      const budgetNum = Number(this.fieldBudget?.value);
      ...
      if (!Number.isFinite(budgetNum) || budgetNum < 1200) {
        this.setState({ quoteError: 'Le budget minimum n’est pas respecté : merci d’indiquer un montant d’au moins 1200 €.' });
        return;
      }
