...
app.post('/api/devis', async (req, res) => {
  ...
  const mailOptions = {
    from: `"SG'pc Site" <${process.env.GMAIL_USER}>`,
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: auto; color: #1a1a2e; background: #f8faff; padding: 32px; border-radius: 8px;">
        ...
        <p style="color: #666; margin-top: 0;">Reçue via le formulaire du site SG'pc</p>
        ...
        <hr style="border: none; border-top: 1px solid #e0e4f0; margin: 24px 0;">
        <p style="font-size: 12px; color: #999;">Message envoyé automatiquement depuis sg-pc.thibaud.site</p>
      </div>
    `,
  };
...
app.listen(PORT, () => {
  console.log(`SG'pc server → http://localhost:${PORT}`);
});
